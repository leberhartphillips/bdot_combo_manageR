cap <- 
  ...
  filter(site %in% c("TS", "CR", "MB", "OD", "TR", "MC")) %>% 
...
cap %>% 
  filter(code == "MX.YY|XX.WY")

# check a random combo to see if it looks good
dplyr::filter(cap, code == "MX.YB|XX.BB") %>% pull(ring, sex)
...
cap2 <- 
  cap[which(grepl(
    "[[FWXM]{1}X\\.[[Y]{1}[[GWRBYO]{1}\\|[[MX]{1}X.[[GWRBYO]{1}[[GWRBYO]{1}",
    cap$code)),]
...
xx[1,] <- c("", "B", "G", "R", "O", "W", "Y")
yy <- 
  data.frame(code=c('M.YB|XX', 'M.YG|XX', 'M.YR|XX', 
                    'M.YO|XX', 'M.YW|XX', 'M.YY|XX'), id <- 1:6)
...
write.table(zz1, "sheets/2024/2024_Mackenzie_Banded_Dotterel_Band_Combinations.txt", 
            na = '', 
            row.names = F, 
